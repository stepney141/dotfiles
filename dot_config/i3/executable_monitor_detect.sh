...
# hdmi_name=$(xrandr --current | awk '/HDMI-A-[0-2] connected/{print $1}')
hdmi_name="HDMI-A-1"
...
do
    ...
    if [ $st_prev != $st_next ]; then
        ...
        if [ $st_next = "connected" ]; then
            xrandr --output eDP --primary --auto --output "HDMI-A-0" --auto --left-of eDP
        else
            ## 外部ディスプレイを接続していない時の設定
            xrandr --output eDP --primary --mode 1920x1200 --pos 1920x0 --rotate normal --output "HDMI-A-0" --off
        fi
    fi
